...
button-size: as-pair box-width / 4 - 1 25
...
draw-infoPanel: compose [pen gray line-width 1 box 1x1 (as-pair box-width - 1 164)]
...
win: layout [
	...
	style button: button button-size font f-font font-color black font-size 8
	style text: text 80
	across
	...
	infoPanel: panel [
		...
		return
		text "Scren size:" t7: textInfo string-sz
		return
		text "Rendered in:" t8: textInfo string-elapse
		return across pad -15x15 space 1x0
		button "Reload" [] button "Transform" [probe length? infoPanel/pane] button "Ceph" [] button font-color crimson "Quit" [quit]
		return pad -15x10
		text 250x50 {Scroll the mouse in any face or drag the orange gel to select an area to display} italic
	]
]
...
win/actors: object [
	...
	if (win/size/x >= svsz/x) or (win/size/y >= svsz/y) [print "Max Size!"];"resized!"
		;if win/size = svsz [print "Max size!"]
	]
]
...
G: (d/offset - c-offset) * iz / c-size
H: d-size * iz / c-size
draw-infoPanel/7: as-pair box-width - 1 infoPanel/size/y - 110
;infoPanel/size
append infoPanel/pane reduce make face! [size: 250x165 type: 'base draw: draw-infoPanel]
